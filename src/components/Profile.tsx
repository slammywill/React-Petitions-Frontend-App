...
const Profile = () => {
    ...
    React.useEffect(() => {
        if (authUser) {
            axios
                .get(BASE_URL + "/users/" + authUser.userId, {
                    headers: {
    ...
    React.useEffect(() => {
        if (authUser) {
            axios
                .get(BASE_URL + "/users/" + authUser.userId + "/image", {
                    responseType: "blob",
                })
                .then((response) => {
    ...
    const validateNewPassword = () => {
        if (
            ...
            setNewPasswordError(
                "Password must be at least 6 characters and different to old password",
            );
    ...
    const handleEditProfile = () => {
        if (authUser) {
            ...
            axios
                .patch(BASE_URL + "/users/" + authUser.userId, data, {
                    headers: {
                        "X-Authorization": authUser.token,
                    },
                })
    ...
    const handleDeleteProfileImage = () => {
        if (authUser) {
            axios
                .delete(BASE_URL + "/users/" + authUser.userId + "/image", {
                    headers: {
                        "X-Authorization": authUser.token,
                    },
                })
            ...
            {authUser && (
                ...
                >
                    <Paper style={paperStyle} elevation={3}>
                        ...
                        <FormHelperText style={{ color: "#e15141" }} id="email-error-text">
                            {emailError}
                        </FormHelperText>
                        <FormControl
                            sx={{ display: "flex", marginTop: enableEdit ? "30px" : 0 }}
                        >
                            <TextField
                        ...
                        <FormHelperText
                            style={{ color: "#e15141" }}
                            id="old-password-error-text"
                        >
                            {oldPasswordError}
                        </FormHelperText>
                        <FormControl
                            style={{ display: "flex", marginTop: enableEdit ? "30px" : 0 }}
                        >
                            <TextField
                                label="New Password"
                                size="medium"
                        ...
                        <Typography
                            variant="subtitle1"
                            sx={{ marginTop: "30px", display: enableEdit ? "block" : "none" }}
                        >
                            Click image to select new picture:
                        </Typography>
                        <div
                            style={{
                                display: "flex",
