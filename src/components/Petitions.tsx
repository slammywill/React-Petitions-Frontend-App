...
const Petitions = () => {
    ...
    const [filteredPetitions, setFilteredPetitions] = React.useState<
        Array<Petition>
    >([]);
    const [paginatedPetitions, setPaginatedPetitions] = React.useState<
        Array<Petition>
    >([]);
    ...
    React.useEffect(() => {
        ...
        if (!isNaN(Number(maxSupportingCost))) {
            filtered = filtered.filter(
                (petition) => petition.supportingCost <= maxSupportingCost,
            );
        }
            ...
            filtered = filtered.filter(
                (petition) =>
                    ...
                    petition.supporters?.some(
                        (supporter) => supporter.supporterId === authUser.userId,
                    ),
            );
        ...
        setPaginatedPetitions(
            filtered.slice(pageSize * (pageNumber - 1), pageSize * pageNumber),
        );
    }, [
        searchQuery,
        selectedCategories,
    ...
    const petition_rows = () =>
        paginatedPetitions.map((petition: Petition) => (
    ...
    const handleMaxSupportingCostChange: React.ChangeEventHandler<
        HTMLInputElement
    > = (event) => {
