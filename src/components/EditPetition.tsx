...
const EditPetition = () => {
    ...
    const updatePetition = () => {
        axios
            .patch(
                BASE_URL + "/petitions/" + petition.petitionId,
                {
                    title: title,
    ...
    const addSupportTier = (title: String, description: String, cost: Number) => {
        if (
            !(
                supportTiers &&
                supportTiers.some((supportTier) => supportTier.title === title)
            )
        ) {
            axios
                .put(
                    BASE_URL + "/petitions/" + petition.petitionId + "/supportTiers/",
                    {
                        title: title,
                        description: description,
                        cost: cost,
    ...
    const noSupporters = (supportTier: SupportTier) => {
        return !(
            supporters &&
            supporters.some(
                (supporter) => supporter.supportTierId === supportTier.supportTierId,
            )
        );
    };
        ...
        if (authUser && authUser.userId && image) {
            ...
            reader.onload = async () => {
                try {
                    ...
                    await axios.put(
                        BASE_URL + "/petitions/" + petition.petitionId + "/image",
                        uint8Array,
                        {
                            headers: {
                    ...
                    <Paper style={paperStyle} elevation={3}>
                        ...
                        <div style={{ display: "flex", justifyContent: "center" }}>
                            <Avatar
                                alt="Profile Picture"
                                variant="rounded"
                                src={image ? URL.createObjectURL(image) : petitionImageUrl}
                                style={{
                                    objectFit: "cover",
                                    width: "400px",
                                    height: "400px",
